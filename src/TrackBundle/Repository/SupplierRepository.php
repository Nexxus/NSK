    ...
    public function findBySearchQuery($query)
    {
        if (is_numeric($query))
        {
            $q = $this->getEntityManager()
                ->createQuery("SELECT c FROM TrackBundle:Supplier c WHERE c.id = ?1 OR c.kvkNr = ?1 OR c.name LIKE ?2");
        }
        else
        {
            $q = $this->getEntityManager()
                ->createQuery("SELECT c FROM TrackBundle:Supplier c WHERE c.email = ?1 OR c.name LIKE ?2");
        }
